...
def log_posterior_probs(x):
	...
	log_p = np.log([.35, .25, .4])
	log_p[0] += -.25 * np.sum(x ** 2) - (np.log(2) + .5 * np.log(np.pi)) * len(x)
	log_p[1] += - np.sum(np.abs(x)) - np.log(2) * len(x)
	log_p[2] += - 2.5 * np.sum(np.log(1 + .25 * x ** 2)) + np.log(.375) * len(x)
